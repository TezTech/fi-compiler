module.exports = function(core) {
	const
		varModifiers = ['add', 'sub', 'mul', 'div', 'mod', 'concat']
		 const varFunctions = ['in', 'length', 'get', 'push', 'drop', 'pop', 'to_address', 'to_contract', 'to_nat', 'to_int', 'to_optional', 'to_pkh', 'to_some', 'to_mutez']
		...
		, NOW: {
		 ...
		 const methods = ['abs', 'add', 'assert', 'concat', 'concat_ws', 'delegate', 'div', 'drop', 'get', 'hash', 'if', 'in', 'isset', 'length', 'mod', 'mul', 'neg', 'new', 'none', 'pack', 'pop', 'push', 'set', 'slice', 'sqr', 'sub', 'throw', 'to_address', 'to_contract', 'to_int', 'to_mutez', 'to_nat', 'to_optional', 'to_pkh', 'to_some', 'transfer', 'unpack', 'verify']

	for (let i = 0; i < methods.length; i++) {
		methodHandlers[methods[i]] = require(`./methods/${methods[i]}`)(core)
	}
	const _compileCode = function(op) {
		var op = op.slice(0); const code = ''; let com = op.shift()
		if (com == 'ext') {
			com = op.shift()
		...
		if (core.literalTypes.indexOf(com) >= 0) {
			...
			const i = core.helper.findInObjArray(core.map.const, 'name', com)
			return {
	...
	return function(code) {
		...
		for (let i = 0; i < code.length; i++) {
			...
			if (line.type) {
				throw 'Code returns a value that is not stored'
			}
