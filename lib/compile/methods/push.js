 ...
 return function(op){
	 var ret = {code : "", type : false};
		...
		if (op.length == 2) {
			...
			ret.code += key.code;
			ret.code += core.compile.ml('dip', val.code + core.compile.ml('some'));
			ret.code += core.compile.ml('diip', map.code);
			ret.code += core.compile.ml('update');
		} else if (op.length == 1) {
			...
			if (ls.type[0] == 'list'){
				...
				ret.code += val.code;
				ret.code += core.compile.ml('dip', ls.code);
				ret.code += core.compile.ml('cons');				
			} else if (ls.type[0] == 'set'){
				if (core.compile.type(val.type[0]) != core.compile.type(ls.type[1])) throw "Invalid value type for push, expects " + ls.type[1] + " not " + val.type[0];
				ret.code += val.code;
				ret.code += core.compile.ml('dip', "PUSH bool True;");
				ret.code += core.compile.ml('diip', ls.code);
				ret.code += core.compile.ml('update');
			} else  throw "Invalid type for push, expects map, big_map, list or set not " + ls.type[0];
		}
		ret.code += core.compile.setter(vName[0]).code;
		return ret
	}
};