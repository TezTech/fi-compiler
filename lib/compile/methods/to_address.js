...
module.exports = function(core) {
	return function(op) {
	 const ret = {
		...
		if (op.length != 1) {
			throw 'Invalid arguments for function to_address, expects 1'
		}
		const a1 = core.compile.code(op.shift())
		if (['contract', 'key_hash', 'key'].indexOf(a1.type[0]) < 0) {
			throw `Invalid argument type for to_address, expects contract, key_hash or key not ${a1.type[0]}`
		}
		ret.code = a1.code
