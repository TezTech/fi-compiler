module.exports = function(core){
	return parseCondition = function(cond){
		...
		var escaped = false, instring = false, fna = [], cfn = '', condArray = [], current = [], ret = '', cc = '', val = '', co = [], cos = [], ret = '', cmp = '', clevel = 0, lhs = false,cs = [],op;
			...
			if (instring){
				...
				if (cc == '('){
					...
					if (['and','or'].indexOf(val) >= 0){
						if (cs.length && co.length) co.push(cs);
						if (cs.length && !co.length) co = cs;
						if (cos.length){
							cos.push(co);
							cos = [val, cos];
						} else {
