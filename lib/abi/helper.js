...
const encodeRawBytes = function(input) {
...
module.exports = function(core) {
	var typevalue = function(p, ts, abi) {
		var p = p.slice(0)
		var ts = ts.slice(0)
		if (typeof p == 'string') {
			p = [[p]]
		...
		if (p.length == 0) {
			...
			if (core.literalTypes.indexOf(p[0][0]) >= 0) {
				return core.compile.literal(p[0][0], ts[0])
			}
			else if (p[0][0] == 'unit') {
				return 'Unit'
			}
			...
			return namedTypevalue(abi.struct[core.helper.findInObjArray(abi.struct, 'name', p[0][0])].type, ts[0], abi)
		}

		let ret = '(Pair '
		ret += typevalue([p.shift()], [ts.shift()], abi)
		ret += ' '
		let e = ''
		while (p.length > 1) {
			ret += '(Pair '
	...
	var namedTypevalue = function(p, ts, abi) {
		var p = p.slice(0)
		if (p.length == 0) {
			return 'Unit'
		}
		...
		while (p.length) {
			const tt = p.shift()
			types.push(tt.type)
			if (typeof ts[tt.name] == 'undefined') {
				throw `Error with JSON ${tt.name}`
			}
			vals.push(ts[tt.name])
		}
		return typevalue(types, vals, abi)
	}
	return {
		packData(d) {
